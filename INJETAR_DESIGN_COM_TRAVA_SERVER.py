...
class InjectDesignLockEngine:
    ...
    def aplicar_layout_com_trava_server(self):
        print(" [1/3] 🎨 Restaurando layout IOTEC e substituindo a brecha do JavaScript por validação Server-Side...")
        
        # O HTML completo fornecido pelo usuário, com a função dispararDownloadPDF() alterada para checagem server-side
...
<meta http-equiv="content-type" content="text/html; charset=UTF-8">
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; transition: all 0.25s ease; }
        ...
        #paypal-button-container { margin-top: 16px; width: 100%; }
...
<body>
        ...
        <div class="controls-toolbar">
            <div style="display: flex; align-items: center; gap: 12px;">
                ...
                <select class="select-custom" id="groupSelect">
                    <option value="setor" selected="selected">Setor Regulatório</option>
                    <option value="modal">Modal de Transporte</option>
    ...
    <div class="modal-overlay" id="paymentModal">
        <div class="modal-card">
            ...
            <div class="checkout-view-initial" id="checkoutView1">
                ...
                <button class="btn-pay-paypal-select" id="btnPayPayPal">Pagar via PayPal Global</button>
            ...
            <div class="checkout-view-paypal" id="checkoutViewPayPal">
                <h3 style="font-size: 15px; font-weight: 700; color: var(--paypal-blue);">PayPal Global Engine</h3>
                <p style="font-size: 11px; color: var(--text-secondary); margin-top: 4px;" id="paypalDocLabel">Aguardando confirmação de gateway...</p>
    ...
    <div class="status-footer">IOTEC Core v16.0 • CNPJ 61.549.037/0001-68</div>
    ...
    <script>
        (function() {
            ...
            function irParaCamada(num) {
                camadaAtual = num;
                document.getElementById("camada-apresentacao").style.display = num === 1 ? "flex" : "none";
                document.getElementById("camada-telemetria").style.display = num === 2 ? "block" : "none";
                document.getElementById("camada-impressao").style.display = num === 3 ? "block" : "none";
            }

            document.getElementById("btnNavCamada").addEventListener("click", function() {
            ...
            // FUNÇÃO REESCRITA COM TRAVA SERVER-SIDE
            function dispararDownloadPDF() {
                const errDiv = document.getElementById("statusErroPix");
                errDiv.style.display = "block";
                errDiv.innerHTML = "🛑 <strong>403 FORBIDDEN: EMISSÃO BLOQUEADA</strong><br>Pagamento não confirmado na base iotec.db. Efetue o Pix e aguarde a validação do servidor.";
            }
...
"""
        with open(self.index_file, "w", encoding="utf-8") as f:
            f.write(html_seguro)
        print("  ✅ Design original gravado em `index.html` com a trava Server-Side ativa.")

    def preparar_dist(self):
        print(" [2/3] 📦 Atualizando pasta isolada 'dist'...")
    ...
    def disparar_deploy(self):
        print(" [3/3] 🚀 Publicando layout completo com trava em produção no Netlify...")
        ...
        cursor.execute('''
            ...
            VALUES ('NETLIFY_FULL_DESIGN_LOCK_ENFORCED', 1, 1, ?)
        ''', (now_utc,))
        conn.commit()
        conn.close()

        print("\n==========================================================================================")
        print(" ✅ LAYOUT OFICIAL RESTAURADO E TOTALMENTE PROTEGIDO EM PRODUÇÃO!")
        print("==========================================================================================")

if __name__ == "__main__":
    engine = InjectDesignLockEngine()
    engine.aplicar_layout_com_trava_server()
