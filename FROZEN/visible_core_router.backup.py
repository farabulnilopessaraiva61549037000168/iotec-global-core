...
, config: Dict[str, Any], log_path: Path) -> None:
        self.config = config
        self.log_path = log_path
    ...
    def validate(self, envelope: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.config["reliability"].get("block_on_untrusted_formula", True):
            if metadata.get("formula_trusted", True) is False:
                blocked = True
                reasons.append("FÃƒÆ'Ã†â€™Ãƒâ€šÃ‚Â³rmula rejeitada.")
...
, log_path: Path) -> None:
    ...
    def pull_back(self, nucleus_name: str, result: Dict[str, Any]) -> Dict[str, Any]:
        append_log(self.log_path, "RETURN", f"{nucleus_name} retornou ao nÃƒÆ'Ã†â€™Ãƒâ€šÃ‚Âºcleo visÃƒÆ'Ã†â€™Ãƒâ€šÃ‚Â­vel.")
...
class SuperIllusionVisibleCore:
    def __init__(self, config: dict, log_path=None) -> None:
        ...
        self.logs_dir = ensure_dir(self.base_dir / self.config.get("paths", {}).get("logs_dir", "logs"))
        self.snapshots_dir = ensure_dir(self.base_dir / self.config.get("paths", {}).get("snapshots_dir", "snapshots"))
    ...
    def dispatch(self, envelope: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if target == "miner":
                ...
                result = {"status": "ignored", "message": f"NÃƒÆ'Ã†â€™Ãƒâ€šÃ‚Âºcleo desconhecido: {target}"}
