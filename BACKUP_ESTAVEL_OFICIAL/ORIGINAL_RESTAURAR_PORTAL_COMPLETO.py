...
class CompletePortalEngine:
    ...
    def montar_html_completo(self):
        print(" [1/3] 🎨 Reconstruindo portal completo (Camadas 1, 2 e 3 + Capa de Rosto + Trava)...")
        
        # HTML Oficial Completo Restaurado
        html_oficial = """<!DOCTYPE html>
<html lang="pt-BR" data-theme="light"><head>
<meta http-equiv="content-type" content="text/html; charset=UTF-8">
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>IOTEC | Global Intelligence &amp; Multimodal Logistics</title>
    <!-- SDK Oficial do PayPal -->
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; transition: all 0.25s ease; }
        ...
        #camada-impressao { display: none; max-width: 1400px; margin: 30px auto; padding: 0 24px; }
        ...
        #paypal-button-container { margin-top: 16px; width: 100%; min-height: 120px; }

        .whatsapp-float { position: fixed; bottom: 24px; right: 24px; background: var(--whatsapp-green); color: #fff; width: 52px; height: 52px; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 10px 25px rgba(37,211,102,0.4); cursor: pointer; z-index: 99; }
        .chat-modal { position: fixed; bottom: 88px; right: 24px; width: 330px; background: var(--bg-card); border: 1px solid var(--bg-card-border); border-radius: 8px; box-shadow: var(--shadow-soft); display: none; flex-direction: column; z-index: 100; overflow: hidden; backdrop-filter: blur(16px); }
        .chat-header { background: var(--accent-pearl); padding: 14px; display: flex; align-items: center; gap: 10px; font-size: 12px; font-weight: 700; }
        .chat-body { padding: 18px; font-size: 12px; color: var(--text-primary); line-height: 1.6; }
...
<body>
    ...
    <div class="header-bar">
        <div style="display: flex; align-items: center;">
            ...
            <span class="badge-layer c1" id="badge-modo">Camada 1: Portal Institucional</span>
        </div>
        <div style="display: flex; gap: 12px; align-items: center;">
            <button class="btn-sm" id="btnThemeToggle">🌗 Black / White</button>
            <button class="btn-sm" id="btnNavCamada" style="border-color: var(--accent-gold); color: var(--accent-gold);">Acessar Telemetria →</button>
        </div>
    </div>

    <!-- CAMADA 1: PORTAL CLEAN LUXURY (FOLHA DE ROSTO) -->
    <div id="camada-apresentacao" style="display: flex;">
        ...
        <p class="main-subtitle">Rastreamento em tempo real de Navios, Aviões, Trens e Caminhões com regulamentação unificada e inteligência de cotação para produtos de alta sensibilidade.</p>
    ...
    <!-- CAMADA 2: TELEMETRIA & PAINEL DINÂMICO -->
    <div id="camada-telemetria" style="display: none;">
        <div class="live-metrics-bar">
            <div class="metric-card">
                ...
                <div class="metric-value" id="valTotal">USD 5,700,000</div>
                <div class="metric-trend">● Atualizado a cada 3s</div>
            </div>
            <div class="metric-card">
                <div class="metric-label">Lotes e Cargas Ativas</div>
                <div class="metric-value" id="valLotes">1,248 Lotes</div>
                <div class="metric-trend">● Telemetria Global Ativa</div>
            </div>
            <div class="metric-card">
                <div class="metric-label">Integridade da Cadeia de Frio</div>
                <div class="metric-value" id="valTemp">99.8% OK</div>
                <div class="metric-trend">● Monitoramento Térmico OK</div>
            </div>
            <div class="metric-card">
                <div class="metric-label">Receita de Impressão de Documentos</div>
                <div class="metric-value" id="valReceita">R$ 48.250,00</div>
                <div class="metric-trend">● Gateways PicPay / PayPal</div>
            </div>
        </div>

        <div class="controls-toolbar">
            <div style="display: flex; align-items: center; gap: 12px;">
                <label style="font-size: 11px; font-weight: 700; text-transform: uppercase; color: var(--text-secondary);">Agrupar Coluna Por:</label>
                <select class="select-custom" id="groupSelect">
                    <option value="setor" selected="selected">Setor Regulatório (Farmacêutico, Proteína, Químico...)</option>
                    <option value="modal">Modal de Transporte (Marítimo, Aéreo, Terrestre)</option>
                    <option value="nenhum">Sem Agrupamento (Visão Contínua)</option>
                </select>
            </div>
            <button class="btn-sm" id="btnIrImpressaoDireto" style="border-color: var(--accent-gold); color: var(--accent-gold);">Ir para Central de Impressão →</button>
        </div>
        ...
        <div class="table-container">
            <table>
                <thead>
                    <tr>
                        <th>Identificador / Carga</th>
                        <th>Setor / Produto</th>
                        <th>Modal / Operação</th>
                        <th>Telemetria / Status</th>
    ...
    <!-- CAMADA 3: CENTRAL DE EMISSÃO DOCUMENTAL EXPANDIDA -->
    <div id="camada-impressao" style="display: none;">
        ...
        <p style="font-size: 13px; color: var(--text-secondary); margin-bottom: 24px;">Emissão e impressão de certidões, licenças e declarações oficiais autenticadas via QR-Code.</p>

        <!-- Seção 1: Aduaneiro & Comércio Exterior -->
        ...
        <!-- Seção 2: Transporte Multimodal -->
        <div class="doc-section-header">2. Transporte Multimodal (Rodoviário, Marítimo, Aéreo, Ferroviário)</div>
        <div class="doc-grid">
            <div class="doc-card-lux">
                <div>
                    <h4>RNTRC - Rodoviário de Cargas</h4>
                    <p>ANTT - Agência Nacional de Transportes • R$ 150,00</p>
                </div>
                <button class="btn-print-lux" data-doc="RNTRC Rodoviário" data-price="150.00">Imprimir Certidão (R$ 150,00)</button>
            </div>
            <div class="doc-card-lux">
                <div>
                    <h4>Declaração de Adimplência Aquaviária</h4>
                    <p>ANTAQ (Marítimo / Fluvial) • R$ 250,00</p>
                </div>
                <button class="btn-print-lux" data-doc="Declaração Aquaviária" data-price="250.00">Imprimir Certidão (R$ 250,00)</button>
            </div>
            <div class="doc-card-lux">
                <div>
                    <h4>CE Mercante / Liberação AFRMM</h4>
                    <p>Siscomex Carga / Marinha Mercante • R$ 150,00</p>
                </div>
                <button class="btn-print-lux" data-doc="CE Mercante / AFRMM" data-price="150.00">Imprimir Certidão (R$ 150,00)</button>
            </div>
            <div class="doc-card-lux">
                <div>
                    <h4>CCT Aéreo / Manifestos AWB</h4>
                    <p>IATA / Receita Federal Aérea • R$ 150,00</p>
                </div>
                <button class="btn-print-lux" data-doc="CCT Aéreo / AWB" data-price="150.00">Imprimir Certidão (R$ 150,00)</button>
            </div>
            <div class="doc-card-lux">
                <div>
                    <h4>TIF / Manifesto Ferroviário</h4>
                    <p>ANTT - Transporte Ferroviário • R$ 150,00</p>
                </div>
                <button class="btn-print-lux" data-doc="TIF Ferroviário" data-price="150.00">Imprimir Certidão (R$ 150,00)</button>
            </div>
        </div>

        <!-- Seção 3: Compliance Fiscal & Licitações -->
        <div class="doc-section-header">3. Compliance Fiscal, Tributário &amp; Licitações</div>
        <div class="doc-grid">
            <div class="doc-card-lux">
                <div>
                    <h4>CND Federal / Dívida Ativa da União</h4>
                    <p>PGFN / Receita Federal do Brasil • R$ 150,00</p>
                </div>
                <button class="btn-print-lux" data-doc="CND Federal PGFN" data-price="150.00">Imprimir Certidão (R$ 150,00)</button>
            </div>
            <div class="doc-card-lux">
                <div>
                    <h4>CRF do FGTS - Certificado de Regularidade</h4>
                    <p>Caixa Econômica Federal • R$ 150,00</p>
                </div>
                <button class="btn-print-lux" data-doc="CRF FGTS CEF" data-price="150.00">Imprimir Certidão (R$ 150,00)</button>
            </div>
            <div class="doc-card-lux">
                <div>
                    <h4>CNDT - Certidão Negativa Trabalhista</h4>
                    <p>TST - Tribunal Superior do Trabalho • R$ 150,00</p>
                </div>
                <button class="btn-print-lux" data-doc="CNDT Trabalhista TST" data-price="150.00">Imprimir Certidão (R$ 150,00)</button>
            </div>
        </div>

        <!-- Seção 4: Sanitário, Ambiental & Cold Chain -->
        <div class="doc-section-header">4. Vigilância Sanitarista, Ambiental &amp; Cold Chain Internacional</div>
        <div class="doc-grid">
            <div class="doc-card-lux">
                <div>
                    <h4>AFE / AE Anvisa - Regularidade Sanitarista</h4>
                    <p>Anvisa - Agência Nacional de Vigilância • R$ 250,00</p>
                </div>
                <button class="btn-print-lux" data-doc="AFE / AE Anvisa" data-price="250.00">Imprimir Certidão (R$ 250,00)</button>
            </div>
            <div class="doc-card-lux">
                <div>
                    <h4>Licença Ambiental de Cargas Perigosas</h4>
                    <p>IBAMA / Órgãos Ambientais • R$ 250,00</p>
                </div>
                <button class="btn-print-lux" data-doc="Licença IBAMA Cargas" data-price="250.00">Imprimir Certidão (R$ 250,00)</button>
            </div>
            <div class="doc-card-lux">
                <div>
                    <h4>Licença IATA CEIV Cold-Chain (Vacinas)</h4>
                    <p>IATA Pharma / Global Cold Chain • USD 250,00</p>
                </div>
                <button class="btn-print-lux" data-doc="Licença Vacinas Cold-Chain" data-price="1250.00">Imprimir Certidão (USD 250)</button>
            </div>
        </div>
    </div>
    ...
    <div class="whatsapp-float" id="btnWhatsAppFloat">
    ...
    <div class="chat-modal" id="chatModal">
        <div class="chat-header">
            <div style="width: 8px; height: 8px; background: var(--whatsapp-green); border-radius: 50%;"></div>
            Agente IOTEC (Atendimento Direto)
        </div>
        <div class="chat-body">
            Olá! Sou o agente do núcleo interno da IOTEC. <strong>Do que você precisa hoje?</strong>
            <br><br>
            CNPJ: 61.549.037/0001-68<br>
            E-mail: iotec.bl@proton.me<br>
            Tel: (88) 99306-4168
        </div>
        <div style="padding: 12px; background: var(--bg-primary); text-align: center;">
            <a href="https://wa.me/5588993064168?text=Ol%C3%A1,%20sou%20cliente%20IOTEC%20e%20preciso%20de%20suporte." target="_blank" class="btn-main btn-dark" style="display: block; font-size: 10px; padding: 10px; text-decoration: none;">Falar com Agente Interno</a>
        </div>
    </div>
    ...
    <script>
        (function() {
            ...
            let dadosCargas = [
                { id: "LOT-VAC-9901", setor: "Farmacêutico / Vacinas", modal: "Aéreo (IATA CEIV)", produto: "Vacinas RNA - Cold Chain", temp: "-52.9 °C", valor: 1250000 },
                { id: "LOT-VAC-9902", setor: "Farmacêutico / Vacinas", modal: "Aéreo (IATA CEIV)", produto: "Imunoglobulinas Biológicas", temp: "+11.0 °C", valor: 890000 },
                { id: "LOT-PROT-402", setor: "Proteína Animal", modal: "Marítimo (Reefer)", produto: "Carne Bovina In Natura", temp: "-12.4 °C", valor: 480000 },
                { id: "LOT-QUIM-112", setor: "Químico / Perigosos", modal: "Terrestre (Rodoviário)", produto: "Insumos Farmacêuticos Ativos", temp: "+15.0 °C", valor: 310000 }
            ];
            ...
            function irParaCamada(num) {
                camadaAtual = num;
                const c1 = document.getElementById("camada-apresentacao");
                const c2 = document.getElementById("camada-telemetria");
                const c3 = document.getElementById("camada-impressao");
                const badge = document.getElementById("badge-modo");
                const btnNav = document.getElementById("btnNavCamada");
                ...
                if (num === 1) {
                    ...
                    badge.className = "badge-layer c1";
                    badge.innerText = "Camada 1: Portal Institucional";
                    btnNav.innerText = "Acessar Telemetria →";
                } else if (num === 2) {
                    c2.style.display = "block";
                    badge.className = "badge-layer c2";
                    badge.innerText = "Camada 2: Telemetria & Painel";
                    btnNav.innerText = "Ir para Impressão →";
                    renderizarTabela();
                } else if (num === 3) {
                    c3.style.display = "block";
                    badge.className = "badge-layer c3";
                    badge.innerText = "Camada 3: Central de Impressão";
                    btnNav.innerText = "← Retornar ao Portal";
                }
            }

            document.getElementById("btnNavCamada").addEventListener("click", function(e) {
                e.preventDefault(); e.stopPropagation();
                irParaCamada(camadaAtual === 1 ? 2 : (camadaAtual === 2 ? 3 : 1));
            });

            document.getElementById("btnIrTelemetria").addEventListener("click", function(e) {
                e.preventDefault(); e.stopPropagation(); irParaCamada(2);
            });

            document.getElementById("btnIrCentralImpressao").addEventListener("click", function(e) {
                e.preventDefault(); e.stopPropagation(); irParaCamada(3);
            });

            document.getElementById("btnIrImpressaoDireto").addEventListener("click", function(e) {
                e.preventDefault(); e.stopPropagation(); irParaCamada(3);
            });

            document.getElementById("groupSelect").addEventListener("change", function(e) {
                e.preventDefault(); e.stopPropagation(); renderizarTabela();
            });
            ...
            function renderizarTabela() {
                ...
                const criterio = document.getElementById("groupSelect").value;
                tbody.innerHTML = "";

                if (criterio === "nenhum") {
                    dadosCargas.forEach(c => tbody.appendChild(criarLinha(c)));
                } else {
                    const grupos = {};
                    dadosCargas.forEach(c => {
                        const chave = c[criterio] || "Outros";
                    ...
                    for (let grupo in grupos) {
                        ...
                        const totalGrupo = grupos[grupo].reduce((sum, item) => sum + item.valor, 0);
                        trGroup.innerHTML = `<td colspan="6">📁 AGRUPAMENTO POR SETOR: <strong>${grupo.toUpperCase()}</strong> (${grupos[grupo].length} LOTES - SUBTOTAL: USD ${totalGrupo.toLocaleString('en-US')})</td>`;
                        tbody.appendChild(trGroup);

                        grupos[grupo].forEach(c => tbody.appendChild(criarLinha(c)));
                    }
                }
            }
            ...
            function criarLinha(c) {
                ...
                tr.innerHTML = `
                    ...
                    <td>${c.setor}<br><small style="color:var(--text-secondary)">${c.produto}</small></td>
                    <td>${c.modal}</td>
                    <td><span style="color:var(--status-ok); font-weight:700;">● ${c.temp}</span></td>
                    <td><strong>USD ${c.valor.toLocaleString('en-US')}</strong></td>
                    <td><button class="btn-sm btn-emitir-item" style="border-color:var(--accent-gold);">Emitir Certidão/Nota</button></td>
                `;
                tr.querySelector(".btn-emitir-item").addEventListener("click", function(e) {
                    e.preventDefault(); e.stopPropagation();
                    docAtualSelecionado = "Certidão do Lote " + c.id;
                    precoAtual = "250.00";
                    abrirModal(docAtualSelecionado, precoAtual);
                });
                return tr;
            }
            ...
            document.getElementById("btnCancelModal").addEventListener("click", function(e) {
                e.preventDefault(); e.stopPropagation(); fecharModal();
            });

            document.getElementById("btnVoltarCheckout1").addEventListener("click", function(e) {
                e.preventDefault(); e.stopPropagation();
                document.getElementById("checkoutView1").style.display = "block";
                document.getElementById("checkoutViewPix").style.display = "none";
            });

            document.getElementById("btnVoltarCheckout2").addEventListener("click", function(e) {
                e.preventDefault(); e.stopPropagation();
                document.getElementById("checkoutView1").style.display = "block";
                document.getElementById("checkoutViewPayPal").style.display = "none";
            });

            document.getElementById("btnPayPicPay").addEventListener("click", function(e) {
                e.preventDefault(); e.stopPropagation();
                document.getElementById("checkoutView1").style.display = "none";
                document.getElementById("checkoutViewPix").style.display = "block";
            });

            document.getElementById("btnPayPayPal").addEventListener("click", function(e) {
                e.preventDefault(); e.stopPropagation();
                document.getElementById("checkoutView1").style.display = "none";
                    ...
                    paypal.Buttons({
                        ...
                        onApprove: function(data, actions) {
                            return actions.order.capture().then(function(details) {
                                alert("Pagamento Aprovado! Aguarde a liberação do documento.");
                                fecharModal();
                            });
                        }
            ...
            document.getElementById("btnConfirmarPagamentoSimulado").addEventListener("click", function(e) {
                ...
                errDiv.innerHTML = "🛑 <strong>403 FORBIDDEN: EMISSÃO BLOQUEADA</strong><br>Aguardando confirmação bancária do Pix na base central.";
            });

            document.getElementById("btnWhatsAppFloat").addEventListener("click", function(e) {
                e.preventDefault(); e.stopPropagation();
            ...
            setInterval(function() {
                if (camadaAtual === 2) {
                    dadosCargas.forEach(c => {
                        if (c.temp.includes("°C")) {
                            let base = parseFloat(c.temp);
                            let variação = (Math.random() * 0.4 - 0.2).toFixed(1);
                            c.temp = (base + parseFloat(variação)).toFixed(1) + " °C";
                        }
                    });
                    renderizarTabela();
                }
...
"""
        with open(self.index_file, "w", encoding="utf-8") as f:
            f.write(html_oficial)
        print("  ✅ Código completo da IOTEC gravado com sucesso em `index.html`.")
    ...
    def disparar_deploy(self):
        print(" [3/3] 🚀 Publicando portal completo com Capa de Rosto no Netlify...")
        ...
        cursor.execute('''
            ...
            VALUES ('NETLIFY_FULL_PORTAL_RESTORED', 1, 1, ?)
        ''', (now_utc,))
        conn.commit()
        conn.close()

        print("\n==========================================================================================")
        print(" ✅ PORTAL ORIGINAL COMPLETO E CAPA DE ROSTO RESTAURADOS COM SUCESSO!")
        print("==========================================================================================")

if __name__ == "__main__":
    engine = CompletePortalEngine()
    engine.montar_html_completo()
