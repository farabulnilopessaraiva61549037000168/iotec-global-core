...
const registrarToque = () => { setToques(prev => { const novos = prev + 1; if (novos === 2) { aoDuploToque(); return 0; } setTimeout(() => setToques(0), 500); return novos; }); };

return ( <div onClick={registrarToque} style={{ fontFamily: 'sans-serif', background: '#fff', color: '#222', height: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', padding: '1rem', }} > {/* BotÃƒÆ'Ã†â€™o Superior de Controle */} <div style={{ display: 'flex', justifyContent: 'flex-end' }}> <button onClick={() => setModoDescanso(false)}>ÃƒÆ'Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€šÃ‚Â¹ÃƒÆ'Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â Encerrar descanso

{/* Painel Central */} <div onClick={aoClicarPainel} style={{ textAlign: 'center', flexGrow: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', fontSize: '2rem' }}> <span>{paineis[painelAtual]}</span> </div> {/* Controles Inferiores */} <div style={{ display: 'flex', justifyContent: 'space-between' }}> <button onClick={() => setModoDescanso(!modoDescanso)}> {modoDescanso ? 'ÃƒÆ'Ã‚Â¢Ãƒâ€šÃ‚ÂÃƒâ€šÃ‚Â¸ÃƒÆ'Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â Pausar RodÃƒÆ'Ã†â€™Ãƒâ€šÃ‚Â­zio' : 'ÃƒÆ'Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Å"Ãƒâ€šÃ‚Â¶ÃƒÆ'Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â Ativar RodÃƒÆ'Ã†â€™Ãƒâ€šÃ‚Â­zio'} </button> <button onClick={() => setSomAtivo(!somAtivo)}> {somAtivo ? 'ÃƒÆ'Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Som Off' : 'ÃƒÆ'Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒâ€¦Ã‚Â  Som On'} </button> <button onClick={() => setVozAtiva(!vozAtiva)}> {vozAtiva ? 'ÃƒÆ'Ã‚Â°Ãƒâ€¦Ã‚Â¸Ãƒâ€¦Ã‚Â½Ãƒâ€šÃ‚Â¤ Voz Off' : 'ÃƒÆ'Ã‚Â°Ãƒâ€¦Ã‚Â¸Ãƒâ€¦Ã‚Â½ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢ÃƒÆ'Ã‚Â¯Ãƒâ€šÃ‚Â¸Ãƒâ€šÃ‚Â Voz On'} </button> </div> </div>
