...
PROJETAR_CLIENTES_MES(INICIO, CRESCIMENTO) {
    return INICIO * (1 + CRESCIMENTO) ** MES_ATUAL;
}

ATIVAR_ESCALONAMENTO() {
    AUMENTAR_SERVIDORES();
    OTIMIZAR_PROCESSOS();
    EXPANDIR_EQUIPE_VIRTUAL();
}

ANALISAR_DEMANDA_POR_PAIS(PAISES) {
    foreach(PAIS in PAISES) {
        PESQUISAR_DEMANDA(PAIS);
        CALCULAR_MARGEM(PAIS);
    }
}

ANALISAR_RISCOS(TIPOS) {
    foreach(TIPO in TIPOS) {
        MONITORAR_RISCO(TIPO);
        GERAR_PLANOS_CONTINGENCIA(TIPO);
    }
}

RELATORIO_MENSAL(FAT, LUCRO, CLIENTES, EXP, RISCO) {
    PRINT("Faturamento Bruto: $" + FAT);
    PRINT("Lucro LÃƒÆ'Ã†â€™Ãƒâ€šÃ‚Â­quido: $" + LUCRO);
    PRINT("Clientes no mÃƒÆ'Ã†â€™Ãƒâ€šÃ‚Âªs: " + CLIENTES);
    PRINT("Status de ExpansÃƒÆ'Ã†â€™o: " + EXP);
    PRINT("Status de Riscos: " + RISCO);
}

// ÃƒÆ'Ã‚Â°Ãƒâ€¦Ã‚Â¸ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒâ€šÃ‚Â¥ Ativar
INICIAR_PROJETO();
